...
let dayPassedNow = Float(myAge) * daysInYear
print("My age is \(myAge) years. After 10 years, I will be \(myAgeInTenYears) years old. From the moment of my birthday have passed approximately \(dayPassedNow) days.")
...
let hypotenuse = sqrt(pow(AC, 2) + pow(CB, 2))
let area = (AC * CB) / 2
let perimeter = AC + CB + hypotenuse

print("hypotenuse = \(hypotenuse), area = \(area), perimeter = \(perimeter)")
